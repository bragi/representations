...
describe ResourceRepresentations::ActiveRecordRepresentation do
  it "when a method with the same name as associated object is called with a block it should create html tags for representations passed inside a block" do
    ...
    user = ResourceRepresentations::representation_for(user, nil, 'user')
    user.profile {|p| p.name.text_field} == %Q{<input type="text" name="user[profile][name]" value="some name" id="name"/>} 
  end
