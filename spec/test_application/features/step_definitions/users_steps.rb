...
When /^I visit this user's show page$/ do
  visit("users/#{@user.id}")
end
...
Then /^with following profile:$/ do |data|
  hash = data.hashes.first
...
Then /^with following tasks:$/ do |data|
...
Then /^the "([^\"]*)" radio button should (not )?be checked$/ do |value, _not|
  _not ? checked = false : checked = true
  response.should have_tag("input[type=radio][checked=?][value=?]", checked, value)
end

Then /^the "(\d{4} \d{1,2} \d{1,2})" should be selected in "(\d+)(?:st|nd|rd|th)" task$/ do |date, number|
  date = date.split(" ")
  date = Date.new(y=date[0].to_i, m=date[1].to_i, d=date[2].to_i)
  @user.tasks[number.to_i-1].due_to.should == date
end

Then /^I should see "([^\"]*)" for "([^\"]*)"$/ do |str1, str2|
  response.should contain(/#{str2}\s*#{str1}/)
end
